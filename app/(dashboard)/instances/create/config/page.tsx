    ...
    <div className="animate-fade-in pb-24">
      ...
      <div className="space-y-6 max-w-5xl">
        ...
        <Card className="animate-slide-up" style={{ animationDelay: '0.25s' }}>
          <CardHeader className="pb-4">
            <SectionHeader icon={Clock} title="自动策略" desc="配置自动关机和释放规则" />
          </CardHeader>
          <CardContent className="space-y-4">
            <FormRow label="自动关机">
              <div className="flex items-center gap-3 flex-wrap">
                <ToggleGroup value={autoShutdown} onChange={setAutoShutdown} options={[{ value: 'none', label: '不关机' }, { value: 'timer', label: '定时关机' }, { value: 'scheduled', label: '指定时间' }]} />
                {autoShutdown === 'timer' && (
                  <div className="flex items-center gap-1.5 text-sm">
                    <Input className="w-20 h-8 text-center" type="number" value={shutdownMinutes} onChange={e => setShutdownMinutes(parseInt(e.target.value) || 0)} />
                    <span className="text-muted-foreground">分钟后</span>
                  </div>
                )}
              </div>
            </FormRow>
            <FormRow label="自动释放">
              <div className="flex items-center gap-3 flex-wrap">
                <ToggleGroup value={autoRelease} onChange={setAutoRelease} options={[{ value: 'none', label: '不释放' }, { value: 'timer', label: '定时释放' }]} />
                {autoRelease === 'timer' && (
                  <div className="flex items-center gap-1.5 text-sm">
                    <Input className="w-20 h-8 text-center" type="number" value={releaseMinutes} onChange={e => setReleaseMinutes(parseInt(e.target.value) || 0)} />
                    <span className="text-muted-foreground">分钟后</span>
                  </div>
                )}
              </div>
